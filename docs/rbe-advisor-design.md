...
> **Implementation note (ENG-1712).** The profile-derived critical path and per-spawn wall times above are not yet wired up. Bazel's BEP `TimingMetrics` carries no critical path, and the compact execution log's per-spawn `SpawnMetrics` timing is not surfaced by the current axl runtime (the `Duration` fields are dropped). Until a `--profile` parser lands, `C` and per-label durations come from the BES `test.xml` durations, and `T_cp` falls back to the **slowest single test's wall time** — a correct lower bound on achievable wall under unbounded parallelism (a single test shard is indivisible), though it excludes the build-action chain that Bazel's own critical path includes. Remotable / remote-cacheable shares are likewise weighted by measured per-label wall, not the (unavailable) per-spawn wall.
